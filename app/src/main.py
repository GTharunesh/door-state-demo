...
class SampleApp(VehicleApp):
    ...
    async def on_hood_change(self, data: DataPointReply):
        ...
        await self.publish_mqtt_event(
            HOOD_OPEN_STATUS_SUBSCRIPTION_TOPIC,
            json.dumps({"hoodStatus": hood_status}),
        )

    async def on_trunk_change(self, data: DataPointReply):
        trunk_status = data.get(self.Vehicle.Body.Trunk.Rear.IsOpen).value
        await self.publish_mqtt_event(
            TRUNK_OPEN_STATUS_SUBSCRIPTION_TOPIC,
            json.dumps({"trunkStatus": trunk_status}),
        )
    ...
    async def on_rear_rh_door_change(self, data: DataPointReply):
        ...
        await self.publish_mqtt_event(
            DOOR_STATE_REAR_RH_SUBSCRIPTION_TOPIC,
            json.dumps({"rearRHDoorStatus": rear_rh_door_status}),
        )
    ...
    async def on_rear_lh_door_change(self, data: DataPointReply):
        ...
        await self.publish_mqtt_event(
            DOOR_STATE_REAR_LH_SUBSCRIPTION_TOPIC,
            json.dumps({"rearLHDoorStatus": rear_lh_door_status}),
        )
    ...
    async def on_get_all_current_status_request_received(self, data: str) -> None:
        ...
        rear_rh_door_status = (
            await self.Vehicle.Cabin.Door.Row2.Right.IsOpen.get()
        ).value
        rear_lh_door_status = (
            await self.Vehicle.Cabin.Door.Row2.Right.IsOpen.get()
        ).value
